...
def rain_24h_comprehensive_sg(sta_ob,grd_fo, save_path=None):
        ...
        if (len(index0[0]) > 0):
            ...
            if (len(index0) == 1):
                ...
                if (i > 0):
                    ...
                    ax.scatter(x, y, c=colors_sta[i], s=pointsize, label=cleves_name[i],linewidths=0.1,edgecolor = "k")
    ...
    hit_rate = nmc_verification.nmc_vf_method.yes_or_no.score.pod(ob, fo, clevs[1:])
    mis_rate = nmc_verification.nmc_vf_method.yes_or_no.score.mr(ob, fo, clevs[1:])
    fal_rate = nmc_verification.nmc_vf_method.yes_or_no.score.pofd(ob, fo, clevs[1:])
...
def rain_24h_comprehensive_chinaland_sg(sta_ob,grd_fo,  save_path=None):
    ...
    ax2 = plt.axes(rect2)
    sta_fo = nmc_verification.nmc_vf_base.function.gxy_sxy.interpolation_nearest(grd_fo, sta_ob_in)
    # print(sta_fo)
    data_name = nmc_verification.nmc_vf_base.get_data_names(sta_ob_in)
    ob = sta_ob_in[data_name[0]].values
    data_name = nmc_verification.nmc_vf_base.get_data_names(sta_fo)
...
def temper_gg(grd_ob,grd_fo,save_path = None):
    ...
    ax1 = plt.axes(rect1)
    mp1 = Basemap(llcrnrlon=grid0.slon, urcrnrlon= grid0.elon,llcrnrlat= grid0.slat, urcrnrlat=grid0.elat)
    add_china_map_2basemap(mp1, ax1, name='province', edgecolor='k', lw=0.3,encoding = 'gbk')
    add_china_map_2basemap(mp1, ax1, name='river', edgecolor='k', lw=0.3, encoding='gbk')
    ...
    ax2 = plt.axes(rect2)
    mp2 = Basemap(llcrnrlon=grid0.slon, urcrnrlon=grid0.elon, llcrnrlat=grid0.slat, urcrnrlat=grid0.elat)
    add_china_map_2basemap(mp2, ax2, name='province', edgecolor='k', lw=0.3,encoding = 'gbk')
    add_china_map_2basemap(mp2, ax2, name='river', edgecolor='k', lw=0.3, encoding='gbk')
    ...
    ax3 = plt.axes(rect3)
    mp3 = Basemap(llcrnrlon=grid0.slon, urcrnrlon=grid0.elon, llcrnrlat=grid0.slat, urcrnrlat=grid0.elat)
    add_china_map_2basemap(mp3, ax3, name='province', edgecolor='k', lw=0.3,encoding = 'gbk')
    add_china_map_2basemap(mp3, ax3, name='river', edgecolor='k', lw=0.3, encoding='gbk')
