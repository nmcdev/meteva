...
class veri_result_set:
    ...
    def get_veri_result(self):
        result = []
        data_names = nmc_verification.nmc_vf_base.get_undim_data_names(self.sta_data_set.sta_data)
        sta_list, para_dict_list_list,para_dict_list_string = self.sta_data_set.get_sta_list()
        for vmethod in self.vmethod_list:
            if vmethod.lower() == "ts":
                for sta in sta_list:
                    _,ts_list = nmc_verification.nmc_vf_product.yes_or_no.ts(sta,self.para1)
                    result.append(ts_list)
            elif vmethod.lower() == "bias":

                for sta in sta_list:
                    _,ts_list = nmc_verification.nmc_vf_product.yes_or_no.bias(sta,self.para1)
                    result.append(ts_list)
            elif vmethod.lower() == "mis_rate":

                for sta in sta_list:
                    _,ts_list = nmc_verification.nmc_vf_product.yes_or_no.mis_rate(sta,self.para1)
                    result.append(ts_list)
            elif vmethod.lower() == "fal_rate":
                for sta in sta_list:
                    _,ts_list = nmc_verification.nmc_vf_product.yes_or_no.fal_rate(sta,self.para1)
                    result.append(ts_list)
            else:
                pass
