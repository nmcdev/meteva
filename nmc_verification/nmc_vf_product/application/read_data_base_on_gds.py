...
def read_stawind(para, filename,station = None, level=None,time=None, dtime=None):
    ...
    if path is None:
        ip, port = nmc_verification.nmc_vf_base.io.read_gds_ip_port(para["ip_port_file"])
        return nmc_verification.nmc_vf_base.io.read_stawind_from_gds(ip,port,filename,station,level,time,dtime)
    else:
        return nmc_verification.nmc_vf_base.io.read_stawind_from_gdsfile(path,  station, level, time,
                                                              dtime)
...
def read_stawind_from_gridwind(para,filename,station):
    ...
    if path is None:
        ip, port = nmc_verification.nmc_vf_base.io.read_gds_ip_port(para["ip_port_file"])
        return nmc_verification.nmc_vf_base(ip,port,filename,station)
    else:
        file1,ft = os.path.splitext(path)
        if ft == ".nc":
            grd = nmc_verification.nmc_vf_base.read_griddata_from_nc(path)
            ...
            return nmc_verification.nmc_vf_base.read_stawind_from_gds_gridwind_file(path,station)