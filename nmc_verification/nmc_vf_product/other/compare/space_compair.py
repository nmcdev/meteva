...
def draw_veri_rain_24(grid_fo,sta_ob,filename = None):
    ...
    ax2 = plt.axes(rect2)
    sta_fo = bt.gsf.linearInterpolation(grid_fo,sta_ob_in)
    ...
    mae = nvb.nvc.score.mae(ob,fo)
    me = nvb.nvc.score.me(ob,fo)
    mse = nvb.nvc.score.mse(ob,fo)
    rmse = nvb.nvc.score.rmse(ob,fo)
    bias_c = nvb.nvc.score.bias(ob,fo)
    cor = nvb.nvc.score.corr(ob,fo)
    hit,mis,fal,co = nvb.nvb.threshold_list.hmfn(ob,fo,clevs[1:])
    ts = nvb.nvb.threshold_list.ts(ob,fo,clevs[1:])
    ets = nvb.nvb.threshold_list.ets(ob,fo,clevs[1:])
    bias = nvb.nvb.threshold_list.bias(ob,fo,clevs[1:])
    hit_rate = nvb.nvb.threshold_list.hit_rate(ob,fo,clevs[1:])
    mis_rate = nvb.nvb.threshold_list.mis_rate(ob,fo,clevs[1:])
    fal_rate = nvb.nvb.threshold_list.fal_rate(ob,fo,clevs[1:])
