...
def get_time_str_list(time_list):
    str1 = meteva.base.tool.time_tools.get_time_str_one_by_one(time_list[0])
    time_str_list = [str1]
    for i in range(1,len(time_list)):
        time_str_list.append(meteva.base.tool.time_tools.get_time_str_one_by_one(time_list[i],time_list[i-1]))
...
def get_title(method,group_by,group_list,model_name,title = None,discription_uni = ""):
    ...
    if group_by is None:
        ...
        if group_by in ["time","ob_time","day","ob_day"]:
            #判断是否等间距
            if g_num == 1:
                time_str = meteva.base.tool.time_tools.get_time_str_one_by_one(group_list[0])
                group_name += time_str
            elif g_num == 2:
                time_str1 = meteva.base.tool.time_tools.get_time_str_one_by_one(group_list[0])
                time_str2 = meteva.base.tool.time_tools.get_time_str_one_by_one(group_list[1],group_list[0])
                group_name += time_str1+"|"+time_str2
            elif g_num == 3:
                time_str1 = meteva.base.tool.time_tools.get_time_str_one_by_one(group_list[0])
                time_str2 = meteva.base.tool.time_tools.get_time_str_one_by_one(group_list[1],group_list[0])
                time_str3 = meteva.base.tool.time_tools.get_time_str_one_by_one(group_list[2], group_list[1])
...
def get_unique_coods(sta):
    ...
    if time0 == time_1:
        ...
        if repete == 1:
            ...
            discription += "time=" + meteva.base.tool.time_tools.get_time_str_one_by_one(sta["time"].values[0])+" "
            not_unique = False
    #print("time")
    #print(time.time() - begin)
    if not_unique:
