...
def pphindcast2d(Object, which_score='ets', time_point=1, obs=1, model=1,
    ...
    for threshold in range(q):
        dat2_X = thresholder.thresholder(x=X, th=thresholds[threshold], func_type='binary', rule=rule, replace_with=0)
        dat2_Xhat = thresholder.thresholder(x=Xhat, th=thresholds[threshold], func_type='binary', rule=rule,
                                            replace_with=0)
        Ix = dat2_X
        Iy = dat2_Xhat
        for level in range(l):
            ...
            sPx = kernel2dsmooth.kernel2dsmooth(Ix, kernel_type='boxcar', n=levels[level],
                                                W=Wlvl, xdim=xdim, Nxy=Nxy)
            maxp = npy.max(sPy)
            res = optimize.minimize_scalar(findthresh,
                                     args=(sPx, sPy, binmat, which_score),
                                     bounds=(0, maxp),
                                     method="bounded")
            Pthresh[level, threshold] = res['x']
            outmat[level, threshold] = -res["fun"]
    out['value'] = outmat
    out['Pthresh'] = Pthresh
