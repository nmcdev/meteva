...
def acc(grd_ob,grd_fo,climate_path = None):
    ...
    if climate_path is None:
        ...
        if not os.path.exists(z500_cli_path):
            ...
            grd_cli_all =  meteva.base.read_griddata_from_nc(z500_cli_path, grid=grid0)
    else:
        grd_cli_all = meteva.base.read_griddata_from_nc(climate_path, grid=grid0)
    ...
    grd_cli =  meteva.base.in_time_list(grd_cli_all, [time_cli])
...
def acc_middle_z500(para_acc):
    ...
    para_acc["climate_path"] = meteva.base.z500_cli
    acc_middle(para_acc)

def acc_middle(para_acc):

    if "climate_path" not in para_acc.keys():
        para_acc["climate_path"] = None

    if para_acc["climate_path"] is None:
        climate_path = meteva.base.z500_cli
        if not os.path.exists(climate_path):
            ...
            try:
                ...
                urllib.request.urlretrieve(url, filename=climate_path)
            except Exception as e:
                print("从github下载地形高度数据失败，请重试，或者手动从\n" + url + "\n下载文件并保存至\n" + climate_path)
                print(e)
                return
    else:
        climate_path = para_acc["climate_path"]
    ...
    if para_acc["step"] is not None:
        ...
        marker = None
    grd_cli_all =  meteva.base.read_griddata_from_nc(climate_path,grid = para_acc["grid"])
    ...
    if os.path.exists(middle_result_path):
        _,file_type = os.path.splitext(middle_result_path)
        if file_type=="parquet":
            tmmsss0 = pd.read_parquet(middle_result_path)
        else:
    ...
    if "time_type" not in para_acc.keys():
        para_acc["time_type"] = "BT"
            ...
            if len(df3.index) ==len(dtime_list):
                ...
                for dh in dtime_list:
                    ...
                    if len(df4.index) ==0:
                        ...
                        time_ob_ut = time_ut + datetime.timedelta(hours=dh)

                        if para_acc["ob_data"]["time_type"] =="UT":
                            time_path = time_ob_ut
                        else:
                            time_path = time_ob_ut + datetime.timedelta(hours=8)

                        time_cli = datetime.datetime(2020,time_ob_ut.month,time_ob_ut.day,time_ob_ut.hour)
                        grd_cli =  meteva.base.in_time_list(grd_cli_all,[time_cli])

                        path0 =  meteva.base.get_path(para_acc["ob_data"]["dir_ob"], time_path)
                        grd_obs = para_acc["ob_data"]["read_method"](path0, grid=para_acc["grid"], time=time_path, dtime=0, data_name="OBS",show = True,**para_acc["ob_data"]["read_para"])
                        if grd_obs is None: continue
                        meteva.base.set_griddata_coords(grd_obs,gtime=[time_ob])

                        grd_obs.values *= para_acc["ob_data"]["multiple"]
                        grd_obs.values -= grd_cli.values

                        if para_fo1["time_type"]=="UT":
                            time_path = time_ut
                        else:
                            time_path = time_ut + datetime.timedelta(hours=8)

                        path1 = meteva.base.get_path(para_fo1["dir_fo"], time_path, dh)
                        grd_fo = para_fo1["read_method"](path1, grid=para_acc["grid"], time=time_path, dtime=dh, data_name=model,show = True,**para_fo1["read_para"])
                        if grd_fo is not None:
                            meteva.base.set_griddata_coords(grd_fo, gtime=[time1])
    ...
    file_type = middle_result_path.split(".")[1]
    if file_type =="parquet":
        tmmss_all.to_parquet(middle_result_path)  # 将结果输出到文件
    else:
        tmmss_all.to_hdf(middle_result_path, "df")  # 将结果输出到文件
