...
def fuzzy(grd_ob,grd_fo,half_window_sizes, thresholds = None,compare=">=", show=False):
    ...
    result = fuzzy_origin(hold,levels=levels,rule=compare,verbose=show)
    return result


def fuzzy_origin(input_object, levels, time_point=1,
          obs=0, model=0, max_n=None, rule=">=", verbose=False):
    result = hoods2d(input_object, which_methods=["fuzzy"], time_point=time_point,
                     obs=obs, model=model, levels=levels, max_n=max_n, rule=rule, verbose=verbose)["fuzzy"]
...
if __name__ == '__main__':
    ...
    look = mem.fuzzy_logic.fuzzy(grd_ob, grd_fo_03,thresholds=[1], half_window_sizes=[8])
    ...
    # look = fuzzy(hold, levels=[3], verbose=True)
    # print(look)
