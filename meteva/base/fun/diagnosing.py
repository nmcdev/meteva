...
def loc_of_max(sta,used_coords = ["dtime"],span = None,keep_all = True,ignore_missing = False):
    '''
    返回站点数据在某些维度上最大值的坐标
    :param sta:
    :param used_coords:
...
def loc_of_min(sta,used_coords = ["dtime"],span = None,keep_all = True,ignore_missing = False):
    sta1 = sta.copy()
    sta1.iloc[:,6:] *= -1
    return loc_of_max(sta1,used_coords=used_coords,span=span,keep_all = keep_all,ignore_missing = ignore_missing)
